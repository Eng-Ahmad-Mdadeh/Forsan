...
class _MoreMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
    ...
    elevation: 1,
    shadowColor: AppColors.black.withValues(alpha: .08),
    color: AppColors.white,
      ...
      children: [
        for (var index = 0; index < _items.length; index++) ...[
          ...
          if (index != _items.length - 1)
            const Divider(height: 1, thickness: .5, color: AppColors.lightGrey),
        ],
      ],
    ),
  );
...
class _MoreTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      minTileHeight: item.subtitle == null ? 62 : 64,
      contentPadding: EdgeInsets.symmetric(horizontal: AppPaddingWidth.p18),
      leading: Container(
        width: AppWidth.w50,
        height: AppWidth.w50,
        decoration: BoxDecoration(
        ...
        child: Icon(item.icon, color: iconColor, size: AppSize.s25),
      ),
      title: Text(
        item.title,
